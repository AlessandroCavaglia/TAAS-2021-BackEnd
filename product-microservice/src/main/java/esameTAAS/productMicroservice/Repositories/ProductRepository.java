...
public interface ProductRepository extends JpaRepository<Product,Long> {
    ...
    @Query("SELECT P FROM Product P WHERE (P.categories LIKE :category) and (P.latitude >= :minLat and P.latitude <= :maxLat and P.longitude >= :minLong and P.longitude <= :maxLong) AND P.enabled = true AND P.id NOT IN (" +
    ...
    @Query("SELECT P FROM Product P WHERE (P.categories LIKE :category1 OR P.categories LIKE :category2) and P.latitude>=:minLat and P.latitude <=:maxLat and P.longitude>=:minLong and P.longitude <=:maxLong AND P.enabled = true AND P.id NOT IN (" +
            "SELECT distinct A.product_id FROM Action A WHERE A.username=:username)")
    ...
    @Query("SELECT P FROM Product P WHERE (P.categories LIKE :category1 OR P.categories LIKE :category2 OR P.categories LIKE :category3) and P.latitude>=:minLat and P.latitude <=:maxLat and P.longitude>=:minLong and P.longitude <=:maxLong AND P.enabled = true AND P.id NOT IN (" +
            "SELECT distinct A.product_id FROM Action A WHERE A.username=:username)")
